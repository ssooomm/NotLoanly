<template>
  <div class="spending-analysis">
    ...
    <div class="category-details">
      ...
      >
        <span>
          <v-badge dot inline :color="chartColors[index]"></v-badge>
          {{ category.name }}
        </span>
...
// 카테고리 매핑
const categoryMap = apiStore.categories;

// 도넛 차트에 표시할 카테고리 순서 정의 (금융 포함)
const doughnutDisplayOrder = [
  "금융",
  "식비",
  "주거 및 통신",
  "교통",
...
const chartColors = inject("categoryColors");
...
const fetchData = async () => {
  try {
    ...
    // 총 지출액 계산
    totalSpending.value = Array.from(expenseMap.values()).reduce(
